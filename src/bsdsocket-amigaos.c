...
static struct socketbase *alloc_socketbase (TrapContext *context)
{
	...
	if ((sb = calloc (sizeof (struct socketbase), 1)) != NULL)
	{
		...
		Printf("%s:%s:%ld -- return pointer: 0x%p\n",__FILE__,__FUNCTION__,__LINE__,sb);
...
static uae_u32 REGPARAM2 bsdsocklib_Open (TrapContext *context)
{
	...
	if ( is_in_hThreads( self ) == FALSE )
	{
		int index = GetHThread( self );

		if ( index>-1 )
		{
			Printf("warning!! thread added to hThreads[] in %s\n",__FUNCTION__);
		}
		else
		{
			Printf("Danger... failed to add to nThreads[] in %s\n",__FUNCTION__);
			return 0;
		}
	}
...
/*
void InspectLibrary(struct Library *lib)
{
	int i;
	const UBYTE *libname = rtarea_lget( (uaecptr) lib -> lib_Node.ln_Name );

	printf("Found %s\n",libname);
	printf("At address: 0x%08lx\n",(ULONG)lib);
	putstr("Version: %d\n",(ULONG)lib->lib_Version);
	printf("Revision: %d\n", (UONG) lib->lib_Revision);

	printf("Jump table:\n");

	for (i = 6; i <= (6)*10; i += 6)
	{
		ULONG *entry = (ULONG *)((ULONG)lib - i);
		pritnf("Offset: %d",-i);
		printf(" ,Entry Address: 0x%08lx", (ULONG) entry);
		printf(" ,entry data: 0x%08lx\n", *entry);
	}
}
*/

void bsdsocket_os41_install (void)
{
	...
	printf("RTAREA_BASE: 0x%08x\n",addr(0));
	printf("datatable: 0x%08x\n",datatable);
	printf("functable: 0x%08x\n",functable);
	printf("init_place: 0x%08x\n",init_place);
	printf("func_place: 0x%08x\n",func_place);
